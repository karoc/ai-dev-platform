...
function Get-ADPCommandHelpLines {
    ...
    if ((Get-UILanguage) -eq "zh-CN") {
        switch ($CommandName) {
            "setup" { return @("用法:", "  adpos setup [-Distro <name>] [-IsoPath <path>] [-SkipIsoDownload] [-SkipDoctor] [-Force] [-NonInteractive] [-NoRegisterCommand]", "", "说明:", "  一键安装入口。它会执行 precheck、ISO 下载、平台引导、初始化、doctor，并默认注册全局 adpos 命令。", "  首次 clone 后仍可在仓库根目录运行 .\setup.cmd；注册完成后可在任意目录运行 adpos。", "  如果当前终端还没有刷新 PATH，请打开新终端，或在仓库根目录临时使用 .\adpos.cmd。", "", "示例:", "  .\setup.cmd", "  adpos setup -IsoPath D:\ISOs\ubuntu-26.04-live-server-amd64.iso", "  adpos setup -SkipIsoDownload") }
            ...
            "quickstart" { return @("用法:", "  adpos quickstart [-Distro <name>] [-IsoPath <path>] [-SkipIsoDownload] [-SkipDoctor] [-Force] [-NonInteractive] [-NoRegisterCommand] [--help-prereqs]", "", "说明:", "  兼容入口；新用户优先使用 .\setup.cmd 或 adpos setup。", "  会扫描前提条件、下载 ISO、初始化平台并运行 doctor。默认注册全局 adpos；使用 -NoRegisterCommand 可跳过。", "", "示例:", "  adpos quickstart", "  adpos quickstart -Force", "  adpos quickstart --help-prereqs") }
    ...
    switch ($CommandName) {
        "setup" { return @("Usage:", "  adpos setup [-Distro <name>] [-IsoPath <path>] [-SkipIsoDownload] [-SkipDoctor] [-Force] [-NonInteractive] [-NoRegisterCommand]", "", "Description:", "  One-click install entry. Runs precheck, ISO download, platform bootstrap, init, doctor, and registers the global adpos command by default.", "  Right after cloning, run .\setup.cmd from the repository root. After registration, run adpos from any directory.", "  If the current terminal has not refreshed PATH yet, open a new terminal or use .\adpos.cmd from the repository root.", "", "Examples:", "  .\setup.cmd", "  adpos setup -IsoPath D:\ISOs\ubuntu-26.04-live-server-amd64.iso", "  adpos setup -SkipIsoDownload") }
