...
function Get-ADPTopLevelCommandRows {
    if ((Get-UILanguage) -eq "zh-CN") {
        return @(
            [pscustomobject]@{ Usage = "adpos setup [-Plan]"; Summary = "一键安装并注册全局 adpos 命令" }
            ...
            [pscustomobject]@{ Usage = "adpos quickstart [-Distro <name>] [-IsoPath <path>] [-SkipIsoDownload] [-SkipDoctor] [-Force] [-NonInteractive] [-NoRegisterCommand] [-Plan] [--help-prereqs]"; Summary = "兼容的一键引导设置入口" }
            ...
            [pscustomobject]@{ Usage = "adpos demo [-Plan]"; Summary = "显示 survival demo 就绪引导，不运行 demo" }
    ...
    return @(
        [pscustomobject]@{ Usage = "adpos setup [-Plan]"; Summary = "One-click install and register the global adpos command" }
        ...
        [pscustomobject]@{ Usage = "adpos quickstart [-Distro <name>] [-IsoPath <path>] [-SkipIsoDownload] [-SkipDoctor] [-Force] [-NonInteractive] [-NoRegisterCommand] [-Plan] [--help-prereqs]"; Summary = "Compatibility guided setup entry" }
        ...
        [pscustomobject]@{ Usage = "adpos demo [-Plan]"; Summary = "Show the survival demo readiness guide without running the demo" }
...
function Get-ADPCommandHelpLines {
    ...
    if ((Get-UILanguage) -eq "zh-CN") {
        switch ($CommandName) {
            "setup" { return @("用法:", "  adpos setup [-Distro <name>] [-IsoPath <path>] [-SkipIsoDownload] [-SkipDoctor] [-Force] [-NonInteractive] [-NoRegisterCommand] [-Plan]", "", "说明:", "  一键安装入口。它会执行 precheck、ISO 下载、平台引导、初始化、doctor，并默认注册全局 adpos 命令。", "  -Plan 只预览 setup/quickstart 步骤，不扫描前提条件、不下载、不安装、不初始化、不运行 doctor、不改全局命令注册。", "  首次 clone 后仍可在仓库根目录运行 .\setup.cmd；注册完成后可在任意目录运行 adpos。", "  如果 Mutagen 是唯一缺失的前提条件，setup 会安装测试过的本地 Mutagen binary 并重新运行 precheck。", "  如果已有全局 adpos 指向其他 checkout，交互模式会询问是否替换；非交互模式会保留现有绑定，除非使用 -Force。", "  如果当前终端还没有刷新 PATH，请打开新终端，或在仓库根目录临时使用 .\adpos.cmd。", "", "示例:", "  .\setup.cmd", "  .\setup.cmd -Plan", "  adpos setup -IsoPath D:\ISOs\ubuntu-26.04-live-server-amd64.iso", "  adpos setup -SkipIsoDownload") }
            ...
            "quickstart" { return @("用法:", "  adpos quickstart [-Distro <name>] [-IsoPath <path>] [-SkipIsoDownload] [-SkipDoctor] [-Force] [-NonInteractive] [-NoRegisterCommand] [-Plan] [--help-prereqs]", "", "说明:", "  兼容入口；新用户优先使用 .\setup.cmd 或 adpos setup。", "  会扫描前提条件、下载 ISO、初始化平台并运行 doctor。默认注册全局 adpos；使用 -NoRegisterCommand 可跳过。", "  -Plan 只预览步骤，不执行 precheck 修复、下载、安装、初始化、doctor 或全局命令注册。", "  如果 Mutagen 是唯一缺失的前提条件，会安装测试过的本地 Mutagen binary 并重新运行 precheck。", "  如果已有全局 adpos 指向其他 checkout，交互模式会询问是否替换；非交互模式会保留现有绑定，除非使用 -Force。", "", "示例:", "  adpos quickstart", "  adpos quickstart -Plan", "  adpos quickstart -Force", "  adpos quickstart --help-prereqs") }
            "precheck" { return @("用法:", "  adpos precheck", "  adpos precheck --help-prereqs", "", "说明:", "  扫描系统前提条件并显示状态表；--help-prereqs 输出完整安装说明。", "", "示例:", "  adpos precheck") }
            "demo" { return @("用法:", "  adpos demo [-Plan]", "", "说明:", "  survival demo 就绪引导；默认和 -Plan 都只输出手动检查路径，不运行 demo。", "  不会启动 VM、创建快照、修改同步、打开 SSH、修改主机配置、PATH、文件、下载或命令注册。", "  如果 VMware 不可用，不要运行 survival demo；不要伪造 snapshot、restore、SSH、evidence chain 或 evidence export 输出。", "  此命令不批准录制、发布、外联、用户招募、testimonial、Discord 或 GitHub Discussions。", "", "文档:", "  docs/zh-CN/demo-script.md", "  docs/zh-CN/survival-validation.md", "", "示例:", "  adpos demo", "  adpos demo -Plan") }
    ...
    switch ($CommandName) {
        "setup" { return @("Usage:", "  adpos setup [-Distro <name>] [-IsoPath <path>] [-SkipIsoDownload] [-SkipDoctor] [-Force] [-NonInteractive] [-NoRegisterCommand] [-Plan]", "", "Description:", "  One-click install entry. Runs precheck, ISO download, platform bootstrap, init, doctor, and registers the global adpos command by default.", "  -Plan previews setup/quickstart steps only; it does not scan prerequisites, download, install, initialize, run doctor, or change global command registration.", "  Right after cloning, run .\setup.cmd from the repository root. After registration, run adpos from any directory.", "  If Mutagen is the only missing prerequisite, setup installs the tested local Mutagen binary and reruns precheck.", "  If global adpos already points to another checkout, interactive setup asks before replacing it; non-interactive setup keeps the existing binding unless -Force is used.", "  If the current terminal has not refreshed PATH yet, open a new terminal or use .\adpos.cmd from the repository root.", "", "Examples:", "  .\setup.cmd", "  .\setup.cmd -Plan", "  adpos setup -IsoPath D:\ISOs\ubuntu-26.04-live-server-amd64.iso", "  adpos setup -SkipIsoDownload") }
        ...
        "quickstart" { return @("Usage:", "  adpos quickstart [-Distro <name>] [-IsoPath <path>] [-SkipIsoDownload] [-SkipDoctor] [-Force] [-NonInteractive] [-NoRegisterCommand] [-Plan] [--help-prereqs]", "", "Description:", "  Compatibility entry; new users should prefer .\setup.cmd or adpos setup.", "  Scans prerequisites, downloads ISO, initializes the platform, and runs doctor. Registers global adpos by default; use -NoRegisterCommand to skip.", "  -Plan previews steps only; it does not perform precheck remediation, download, install, initialize, run doctor, or change global command registration.", "  If Mutagen is the only missing prerequisite, it installs the tested local Mutagen binary and reruns precheck.", "  If global adpos already points to another checkout, interactive setup asks before replacing it; non-interactive setup keeps the existing binding unless -Force is used.", "", "Examples:", "  adpos quickstart", "  adpos quickstart -Plan", "  adpos quickstart -Force", "  adpos quickstart --help-prereqs") }
        ...
        "demo" { return @("Usage:", "  adpos demo [-Plan]", "", "Description:", "  Survival demo readiness guide. The default command and -Plan both print the manual readiness path without running the demo.", "  Does not start VMs, create snapshots, change sync, open SSH, change host configuration, PATH, files, downloads, or command registration.", "  If VMware is unavailable, do not run the survival demo. Do not fake snapshot, restore, SSH, evidence chain, or evidence export output.", "  This command does not approve recording, publishing, outreach, user recruitment, testimonials, Discord, or GitHub Discussions.", "", "Docs:", "  docs/demo-script.md", "  docs/survival-validation.md", "", "Examples:", "  adpos demo", "  adpos demo -Plan") }
