...
function Get-ADPCommandHelpLines {
    ...
    if ((Get-UILanguage) -eq "zh-CN") {
        switch ($CommandName) {
            ...
            "quickstart" { return @("用法:", "  adpos quickstart [-Distro <name>] [-IsoPath <path>] [-SkipIsoDownload] [-SkipDoctor] [-Force] [-NonInteractive] [-NoRegisterCommand] [--help-prereqs]", "", "说明:", "  兼容入口；新用户优先使用 .\setup.cmd 或 adpos setup。", "  会扫描前提条件、下载 ISO、初始化平台并运行 doctor。默认注册全局 adpos；使用 -NoRegisterCommand 可跳过。", "  如果 Mutagen 是唯一缺失的前提条件，会安装测试过的本地 Mutagen binary 并重新运行 precheck。", "  如果已有全局 adpos 指向其他 checkout，交互模式会询问是否替换；非交互模式会保留现有绑定，除非使用 -Force。", "", "示例:", "  adpos quickstart", "  adpos quickstart -Force", "  adpos quickstart --help-prereqs") }
