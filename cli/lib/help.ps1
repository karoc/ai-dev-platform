...
function Get-ADPTopLevelCommandRows {
    if ((Get-UILanguage) -eq "zh-CN") {
        return @(
            ...
            [pscustomobject]@{ Usage = "adpos isolate [-Plan] [-Namespace <name>]"; Summary = "预览多 checkout 本机隔离配置" }
    ...
    return @(
        ...
        [pscustomobject]@{ Usage = "adpos isolate [-Plan] [-Namespace <name>]"; Summary = "Plan multi-checkout local isolation settings" }
...
function Get-ADPCommandHelpLines {
    ...
    if ((Get-UILanguage) -eq "zh-CN") {
        switch ($CommandName) {
            ...
            "isolate" { return @("用法:", "  adpos isolate [-Plan] [-Namespace <name>]", "", "说明:", "  预览第二个 checkout 的本机隔离配置，不修改 configs\local.json。", "  输出建议的 platform.runtime_namespace、workspace_root、vm_store、provider vm_store 和 runtime static_ip。", "  如果全局 adpos 指向其他 checkout，请在当前仓库根目录运行 .\adpos.cmd isolate -Plan。", "", "示例:", "  adpos isolate -Plan", "  .\adpos.cmd isolate -Plan -Namespace v2") }
