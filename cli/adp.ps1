...
function Show-Help {
    ...
    Write-Host "  adp up <runtime>               Start a runtime (frontend|backend|agent)"
    Write-Host "  adp stop <runtime>             Stop a runtime"
    Write-Host "  adp sync status                Show workspace sync status"
    Write-Host "  adp network apply <rt|all>     Apply configured static IP networking"
    ...
    Write-Host "  adp doctor                     Run diagnostics"
    Write-Host "  adp destroy <runtime>          Destroy a runtime"
