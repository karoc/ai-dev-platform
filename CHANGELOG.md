...
- Improved sync diagnostics for existing Mutagen sessions: `sync status` now prints an ADP runtime summary before the raw Mutagen list, `status` reports unhealthy or endpoint-mismatched sessions, `doctor` treats unusable sessions as issues, and `sync start <runtime>` no longer treats a same-name stale or halted session as success.
