...
- Improved `adp doctor -FixMutagen` first-run remediation output with explicit install phases, download source/target, connection and hard timeout guidance, controlled download-process termination, temporary archive downloads, reuse of existing archives, invalid-archive retry, and manual recovery guidance if the download fails.
