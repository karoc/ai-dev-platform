...
$knownCommands = "setup|uninstall|iso|quickstart|init|up|status|capabilities|isolate|stop|sync|workspace|network|snapshot|restore|logs|doctor|destroy|precheck|demo|help|version|validate|completion|serve|run|sandbox"
...
Assert-Contains -Name "English README command reference includes sync list" -Text $readme -Pattern 'adpos sync status[\s\S]*adpos sync list[\s\S]*adpos sync start <frontend\|backend\|agent\|sandbox>[\s\S]*adpos sync stop <frontend\|backend\|agent\|sandbox>'
Assert-Contains -Name "Chinese README command reference includes sync list" -Text $readmeZh -Pattern 'adpos sync status[\s\S]*adpos sync list[\s\S]*adpos sync start <frontend\|backend\|agent\|sandbox>[\s\S]*adpos sync stop <frontend\|backend\|agent\|sandbox>'
foreach ($case in @(
    ...
    @{ Name = "demo readiness guide command"; Pattern = 'adpos demo \[-Plan\]' },
