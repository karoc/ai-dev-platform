...
Assert-Contains -Name "workspace docs explain shell-only dogfood is enough for first-run lifecycle validation" -Text $workspaceDocs -Pattern 'minimal POSIX shell project[\s\S]*workspace lifecycle end to end[\s\S]*without browser downloads or package installation'
Assert-Contains -Name "Chinese workspace docs explain shell-only dogfood is enough for first-run lifecycle validation" -Text $workspaceDocsZh -Pattern '一个最小的 POSIX shell 项目[\s\S]*workspace lifecycle[\s\S]*不需要下载浏览器或安装额外 packages'
