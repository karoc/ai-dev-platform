...
Assert-Contains -Name "mutagen sync start blocks stale session before rewriting SSH alias" -Text (Read-Text "adapters\windows\mutagen\mutagen.ps1") -Pattern 'Get-SyncSessionInfo[\s\S]*Existing sync session[\s\S]*Remediation: adp sync stop[\s\S]*Write-ErrorLog -Message "Mutagen sync session[\s\S]*Set-MutagenSSHHostConfig'
...
Assert-Contains -Name "workspace dashboard uses bilingual UI output" -Text $workspace -Pattern 'Write-UIHost[\s\S]*?-English[\s\S]*?-Chinese[\s\S]*?Workspace dashboard:[\s\S]*?工作区仪表盘:[\s\S]*?Dashboard only:[\s\S]*?仅仪表盘查看：[\s\S]*?Overview:[\s\S]*?概览:[\s\S]*?Project readiness:[\s\S]*?项目就绪状态:[\s\S]*?Milestone checkpoints:[\s\S]*?里程碑检查点:[\s\S]*?Evaluation hooks:[\s\S]*?评估钩子:[\s\S]*?Task lifecycle:[\s\S]*?任务生命周期:'
Assert-Contains -Name "workspace dashboard empty states are bilingual" -Text $workspace -Pattern 'Write-WorkspaceCheck[\s\S]*?-ChineseName[\s\S]*?\"清单\"[\s\S]*?\"状态\"[\s\S]*?\"项目\"[\s\S]*?\"里程碑\"[\s\S]*?\"评估\"[\s\S]*?\"任务\"[\s\S]*?none configured[\s\S]*?未配置'
