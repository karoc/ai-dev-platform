...
Assert-Contains -Name "mutagen install has watched progress and timeout guidance" -Text (Read-Text "adapters\windows\mutagen\mutagen.ps1") -Pattern 'function\s+Invoke-MutagenArchiveDownload[\s\S]*Downloading Mutagen archive[\s\S]*ConnectionTimeoutSeconds 30[\s\S]*OperationTimeoutSeconds 300[\s\S]*manually download[\s\S]*Installing Mutagen locally[\s\S]*\[1/5\] Preparing local tool directory[\s\S]*\[5/5\] Verifying Mutagen version'
