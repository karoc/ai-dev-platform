...
$workspaceReport = Read-Text "cli\commands\workspace\report.ps1"
$workspaceRecipes = Read-Text "cli\commands\workspace\recipes.ps1"
$workspaceGuides = Read-Text "cli\commands\workspace\guides.ps1"
$workspaceReportMarkdown = Read-Text "cli\commands\workspace\report-markdown.ps1"
...
$workspaceSource = Expand-WorkspaceModule -Source $workspaceSource -ModuleName "report.ps1" -ModuleText $workspaceReport
$workspaceSource = Expand-WorkspaceModule -Source $workspaceSource -ModuleName "recipes.ps1" -ModuleText $workspaceRecipes
$workspaceSource = Expand-WorkspaceModule -Source $workspaceSource -ModuleName "guides.ps1" -ModuleText $workspaceGuides
$workspaceSource = Expand-WorkspaceModule -Source $workspaceSource -ModuleName "report-markdown.ps1" -ModuleText $workspaceReportMarkdown
...
Assert-Contains -Name "CLI catches PowerShell parameter binding errors" -Text $cli -Pattern 'function\s+Write-ADPCommandArgumentError[\s\S]*Invalid arguments for command:[\s\S]*Run ''\$commandText --help'' for usage[\s\S]*catch \[System\.Management\.Automation\.ParameterBindingException\]'
...
Assert-Contains -Name "CLI registers demo command" -Text $cli -Pattern '\$validCommands\s*=\s*@\([\s\S]*"demo"'
...
Assert-Contains -Name "CLI help includes setup command in English" -Text $cliHelp -Pattern 'adpos setup \[-Plan\]"; Summary = "One-click install and register the global adpos command'
Assert-Contains -Name "CLI help includes setup command in Chinese" -Text $cliHelp -Pattern 'adpos setup \[-Plan\]"; Summary = "一键安装并注册全局 adpos 命令'
...
Assert-Contains -Name "CLI help includes demo readiness guide command" -Text $cliHelp -Pattern 'adpos demo \[-Plan\]"; Summary = "Show the survival demo readiness guide without running the demo'
...
Assert-Contains -Name "completion includes setup isolate demo and uninstall commands" -Text $completion -Pattern '"setup"[\s\S]*"isolate"[\s\S]*"demo"[\s\S]*"uninstall"[\s\S]*complete -F _adpos_completion adpos adpos\.cmd'
