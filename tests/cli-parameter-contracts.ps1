...
Assert-Contains -Name "vm factory explains long autoinstall wait" -Text $factory -Pattern 'Waiting for Ubuntu autoinstall to finish[\s\S]*Typical duration is 15-45 minutes[\s\S]*installed-system user/key and provision marker'
Assert-Contains -Name "vm factory reports autoinstall progress instead of vague waiting" -Text $factory -Pattern 'Autoinstall in progress[\s\S]*SSH is up, waiting for installed-system user/key[\s\S]*Autoinstall confirmation timed out'
...
Assert-Contains -Name "status reports seed network drift" -Text (Read-Text "cli\commands\status.ps1") -Pattern 'network drift:[\s\S]*seed uses[\s\S]*rebuild this runtime'
...
Assert-Contains -Name "doctor reports seed network drift" -Text $doctor -Pattern 'seed network drift[\s\S]*Rebuild \$name or update guest networking'
