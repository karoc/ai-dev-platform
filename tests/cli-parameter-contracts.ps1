...
Assert-Contains -Name "status reports unhealthy sync sessions" -Text (Read-Text "cli\commands\status.ps1") -Pattern 'Get-StatusSyncState[\s\S]*Get-SyncSessionInfo[\s\S]*wrong-local[\s\S]*wrong-remote[\s\S]*unhealthy[\s\S]*sync fix:\s+adp sync stop \$TargetRuntime; adp sync start \$TargetRuntime'
Assert-Contains -Name "status distinguishes stale session before runtime creation" -Text (Read-Text "cli\commands\status.ps1") -Pattern '\$RuntimeCreated[\s\S]*stale-session[\s\S]*old Mutagen session exists[\s\S]*sync cleanup:\s+adp sync stop \$TargetRuntime[\s\S]*sync next:\s+adp up \$TargetRuntime; adp sync start \$TargetRuntime'
...
Assert-Contains -Name "doctor reports unhealthy Mutagen sessions" -Text $doctor -Pattern 'Get-SyncSessionInfo[\s\S]*Mutagen session[\s\S]*Remediation: \.\\cli\\adp\.ps1 sync stop \$name; \.\\cli\\adp\.ps1 sync start \$name'
Assert-Contains -Name "doctor treats uncreated runtime stale sync as info cleanup" -Text $doctor -Pattern 'stale before runtime creation[\s\S]*Cleanup stale session: \.\\cli\\adp\.ps1 sync stop \$name[\s\S]*Create runtime before starting sync: \.\\cli\\adp\.ps1 up \$name; \.\\cli\\adp\.ps1 sync start \$name'
...
Assert-Contains -Name "mutagen sync start blocks stale session before rewriting SSH alias" -Text (Read-Text "adapters\windows\mutagen\mutagen.ps1") -Pattern 'Get-SyncSessionInfo[\s\S]*Existing sync session[\s\S]*Remediation: adp sync stop[\s\S]*throw "Mutagen sync session[\s\S]*Set-MutagenSSHHostConfig'
