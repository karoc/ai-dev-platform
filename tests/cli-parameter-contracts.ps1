...
Assert-Contains -Name "completion includes setup and uninstall commands" -Text $completion -Pattern '"setup"[\s\S]*"uninstall"[\s\S]*complete -F _adpos_completion adpos adpos\.cmd'
...
Assert-Contains -Name "workspace task mark validated boundary" -Text $workspace -Pattern 'validated records an external validation result \(status: passed\)'
Assert-Contains -Name "workspace task mark validation_failed boundary" -Text $workspace -Pattern 'validation_failed records an external validation result \(status: failed\)'
Assert-Contains -Name "workspace task mark external validation state path" -Text $workspace -Pattern 'Set-WorkspaceTaskExternalValidation[\s\S]*ValidationStatus'
Assert-Contains -Name "workspace task mark validation_failed in validStates" -Text $workspace -Pattern '\$validStates\s*=\s*@\("prepared", "checkpointed", "checkpoint-waived", "running", "validated", "validation_failed"'
