...
#
# Usage: pwsh -File tests/evidence-chain.ps1
...
# When invoked directly (not via Pester dot-source), run the tests.
# Pester 3.x compatible: dot-source ourselves to load test definitions,
# then invoke Pester to run them. Guard against re-entrance with env var.
if ($MyInvocation.InvocationName -ne '.' -and -not $env:ADP_PESTER_RUNNING) {
    $env:ADP_PESTER_RUNNING = '1'
    try {
        . $PSCommandPath  # dot-source to register Describe/It blocks
        $result = Invoke-Pester -EnableExit:$false
        if ($result -and $result.FailedCount -gt 0) {
            Write-Host "Evidence chain tests FAILED: $($result.FailedCount) failed, $($result.PassedCount) passed" -ForegroundColor Red
            exit 1
        }
        Write-Host "Evidence chain tests OK: $($result.PassedCount) passed, $($result.FailedCount) failed" -ForegroundColor Green
        exit 0
    } finally {
        Remove-Item Env:\ADP_PESTER_RUNNING -ErrorAction SilentlyContinue
    }
}
