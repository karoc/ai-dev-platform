...
function Write-ADPSshAliasOwnershipGuidance {
    ...
    Write-UIHost -English "  note:     SSH aliases are global in the Windows user SSH config; two identical runtime resources cannot own the same alias at once." -Chinese "  说明:   SSH alias 位于 Windows 用户级 SSH config；两个相同 runtime resource 不能同时拥有同一个 alias。" -ForegroundColor Yellow
}
