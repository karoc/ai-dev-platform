...
欢迎使用 ADP-OS。本指南将带你完成首次设置——从零到运行中的开发虚拟机——大约需要 30 分钟。
...
每个阶段完成时都会显示输出。首次启动需要 5–10 分钟——之后的启动会快得多。
...
**症状：** `adp doctor` 显示 `[FAIL] VMware Workstation`。
...
**症状：** `adp doctor` 显示 `[FAIL] WSL` 或 `[FAIL] WSL xorriso`。
...
**症状：** `adp up frontend` 运行一段时间后显示 cloud-init provisioning 超时错误。
