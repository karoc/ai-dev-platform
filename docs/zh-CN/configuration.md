...
该命令会探测 host `VMnet8`，预览目标 `platform.network.vmware_nat` 和 `topology.<runtime>.static_ip` 值；不带 `-Plan` 时只会写入被忽略的 `configs\local.json` override。当 host 探测不可用时，仍然可以手动编辑。如有需要，请在 VMware Workstation 的 Virtual Network Editor 中确认真实 NAT 子网；参见[网络说明](networking.md#前置条件)。
