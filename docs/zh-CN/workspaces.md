...
- `commit`：打印 commit-readiness gate、validation context、review state、diff 检查、暂存和提交边界，但不会 stage 或 commit 文件。只有 validation 已通过、snapshot gate 没有阻塞，并且本地 task state 已标记为 `reviewed` 或 `committed` 时，task 才会被视为 commit-ready。
