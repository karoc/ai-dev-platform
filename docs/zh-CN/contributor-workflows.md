...
1. 运行 `adpos workspace task validate docs-copy-edit -Execute -Plan`。
2. 计划正确后，运行 `adpos workspace task validate docs-copy-edit -Execute`。
3. 运行 `adpos workspace task review docs-copy-edit`。
...
1. 使用 `adpos workspace task validate frontend-browser-acceptance -Execute -Plan` 预览 validation。
...
1. 运行 `adpos workspace task snapshot broad-agent-refactor`。
...
1. 运行 `adpos workspace dashboard` 做快速 health scan。
2. 运行 `adpos workspace report` 检查 release decision、governance loop、decision queues 和 stale-task remediation。
3. 当 decision 需要复制到 pull request、release note 或 handoff 时，运行 `adpos workspace report -Markdown`。
...
- 已运行的 validation commands，以及是否通过 `adpos workspace task validate -Execute` 执行。
