...
- `release candidate`：所有 task 都是 release candidate，validation 已通过，review 已记录，snapshot gate 未阻塞，并且 task governance 字段已填写。
- `release blocked`：至少一个 task 被 snapshot gate 或失败 validation 阻塞。
...
- `create snapshot`：在 execution、review 或 commit 前创建 checkpoint，或在 ADP-OS 外部显式豁免。
...
8. 确认高风险 task 的 snapshot gate 已 ready，或已经在外部显式豁免。
9. 只提交显示为 `ready to commit` 和 `release candidate` 的 task。
...
- 对高风险工作使用 `tasks[].requires_snapshot` 和 `tasks[].snapshot`。
