...
- 改进 `adp doctor -FixMutagen` 的 first-run 修复输出，安装时会显示明确阶段、下载 source/target、连接和 hard timeout、受控终止下载进程、临时 archive 下载、复用已有 archive、archive 无效时重新下载，以及下载失败时的手动恢复指引。
